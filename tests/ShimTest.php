...
test('cal days in month french', function () {
    foreach ([3, 4] as $year) {
...
test('cal days in month french bug67976', function () {
    expect(cal_days_in_month(\CAL_FRENCH, 13, 14))->toBe(Shim::calDaysInMonth(\CAL_FRENCH, 13, 14));
});
test('cal days in month french invalid month1', function () {
    $this->expectException('ValueError');
    $this->expectExceptionMessage('Invalid date');

    Shim::calDaysInMonth(\CAL_FRENCH, 14, 10);
});
test('cal days in month french invalid month2', function () {
    $this->expectException('ValueError');
    $this->expectExceptionMessage('Invalid date');

    cal_days_in_month(\CAL_FRENCH, 14, 10);
});
test('cal days in month french zero year1', function () {
    $this->expectException('ValueError');
    $this->expectExceptionMessage('Invalid date');

    Shim::calDaysInMonth(\CAL_FRENCH, 1, 0);
});
test('cal days in month french zero year2', function () {
    $this->expectException('ValueError');
    $this->expectExceptionMessage('Invalid date');

    cal_days_in_month(\CAL_FRENCH, 1, 0);
});
test('cal days in month french negative year1', function () {
    $this->expectException('ValueError');
    $this->expectExceptionMessage('Invalid date');

    Shim::calDaysInMonth(\CAL_FRENCH, 1, -1);
});
test('cal days in month french negative year2', function () {
    $this->expectException('ValueError');
    $this->expectExceptionMessage('Invalid date');

    cal_days_in_month(\CAL_FRENCH, 1, -1);
});
test('cal days in month french high year1', function () {
    $this->expectException('ValueError');
    $this->expectExceptionMessage('Invalid date');

    Shim::calDaysInMonth(\CAL_FRENCH, 1, 15);
});
test('cal days in month french high year2', function () {
    $this->expectException('ValueError');
    $this->expectExceptionMessage('Invalid date');
...
test('cal days in month gregorian invalid month1', function () {
    $this->expectException('ValueError');
    $this->expectExceptionMessage('Invalid date');

    Shim::calDaysInMonth(\CAL_GREGORIAN, 13, 2014);
});
test('cal days in month gregorian invalid month2', function () {
    $this->expectException('ValueError');
    $this->expectExceptionMessage('Invalid date');

    cal_days_in_month(\CAL_GREGORIAN, 13, 2014);
});
test('cal days in month gregorian invalid year1', function () {
    $this->expectException('ValueError');
    $this->expectExceptionMessage('Invalid date');

    Shim::calDaysInMonth(\CAL_GREGORIAN, 1, 0);
});
test('cal days in month gregorian invalid year2', function () {
    $this->expectException('ValueError');
    $this->expectExceptionMessage('Invalid date');
...
test('cal days in month jewish invalid month1', function () {
    $this->expectException('ValueError');
    $this->expectExceptionMessage('Invalid date');

    Shim::calDaysInMonth(\CAL_JEWISH, 14, 2014);
});
test('cal days in month jewish invalid month2', function () {
    $this->expectException('ValueError');
    $this->expectExceptionMessage('Invalid date');

    cal_days_in_month(\CAL_JEWISH, 14, 2014);
});
test('cal days in month jewish invalid year1', function () {
    $this->expectException('ValueError');
    $this->expectExceptionMessage('Invalid date');

    Shim::calDaysInMonth(\CAL_JEWISH, 1, 0);
});
test('cal days in month jewish invalid year2', function () {
    $this->expectException('ValueError');
    $this->expectExceptionMessage('Invalid date');

    cal_days_in_month(\CAL_JEWISH, 1, 0);
});
test('cal days in month julian invalid month1', function () {
    $this->expectException('ValueError');
    $this->expectExceptionMessage('Invalid date');

    Shim::calDaysInMonth(\CAL_JULIAN, 13, 2014);
});
test('cal days in month julian invalid month2', function () {
    $this->expectException('ValueError');
    $this->expectExceptionMessage('Invalid date');

    cal_days_in_month(\CAL_JULIAN, 13, 2014);
});
test('cal days in month julian invalid year1', function () {
    $this->expectException('ValueError');
    $this->expectExceptionMessage('Invalid date');

    Shim::calDaysInMonth(\CAL_JULIAN, 1, 0);
});
test('cal days in month julian invalid year2', function () {
    $this->expectException('ValueError');
    $this->expectExceptionMessage('Invalid date');

    cal_days_in_month(\CAL_JULIAN, 1, 0);
});
test('cal days in month invalid calendar1', function () {
    $this->expectException('ValueError');
    $this->expectExceptionMessage('cal_days_in_month(): Argument #1 ($calendar) must be a valid calendar ID');

    Shim::calDaysInMonth(999, 1, 1);
});
test('cal days in month invalid calendar2', function () {
    $this->expectException('ValueError');
    $this->expectExceptionMessage('cal_days_in_month(): Argument #1 ($calendar) must be a valid calendar ID');

    cal_days_in_month(999, 1, 1);
});
test('cal from jd french', function () {
    // 0/0/0
...
test('cal from jd gregorian', function () {
    // 0/0/0
...
test('cal from jd jewish', function () {
    // 0/0/0
...
test('cal from jd julian', function () {
    // 0/0/0
...
test('french to jd out of range', function () {
    expect(frenchtojd(1, 1, 0))->toBe(Shim::frenchToJd(1, 1, 0));
});
test('gregorian to jd', function () {
...
test('jd day of week sunday', function () {
...
test('jd day of week monday', function () {
...
test('jd day of week tuesday', function () {
...
test('jd day of week wednesday', function () {
...
test('jd day of week thursday', function () {
...
test('jd day of week friday', function () {
...
test('jd day of week saturday', function () {
...
test('jd day of week negative', function () {
    expect(6)->toBe(Shim::jdDayOfWeek(-2, 0));
});
test('jd day of week invalid mode', function () {
    $julian_day = gregoriantojd(8, 31, 2014);
...
test('jd month name french', function () {
...
test('jd month name gregorian', function () {
...
test('jd month name jewish', function () {
...
test('jd month name julian', function () {
    for ($month = 1; $month <= 12; ++$month) {
...
test('jd month name invalid mode', function () {
...
test('jd to french edge cases', function () {
    expect(Shim::calFromJd(2375839, \CAL_FRENCH))->toBe(cal_from_jd(2375839, \CAL_FRENCH));
...
test('jd to french out of range', function () {
...
test('jd to gregorian edge cases', function () {
...
test('jd to jewish edge cases', function () {
    expect(Shim::calFromJd(347997, \CAL_JEWISH))->toBe(cal_from_jd(347997, \CAL_JEWISH));
...
test('jd to jewish hebrew out of range low1', function () {
    $this->expectException('ValueError');
...
test('jd to jewish hebrew out of range low2', function () {
    $this->expectException('ValueError');
    $this->expectExceptionMessage('Year out of range (0-9999)');
...
test('jd to jewish hebrew out of range high1', function () {
    $this->expectException('ValueError');
    $this->expectExceptionMessage('Year out of range (0-9999)');

    Shim::jdToJewish(4000076, true, 0);
});
test('jd to jewish hebrew out of range high2', function () {
    $this->expectException('ValueError');
    $this->expectExceptionMessage('Year out of range (0-9999)');

    jdtojewish(4000076, true, 0);
});
test('jd to julian', function () {
...
test('jd to julian edge cases', function () {
...
test('jd to unix edge cases lower limit', function () {
    $lower_limit = 2440588;
...
test('jd to unix edge cases upper limit', function () {
...
test('unix to jd edge cases', function () {
